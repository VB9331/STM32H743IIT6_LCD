/***
   ...
   *  @description
	*
	*	实验平台：反客STM32H743IIT6核心板 （型号：FK743M2-IIT6）+ 4.3寸RGB液晶屏(屏幕型号RGB043M1-480*272)
	*	淘宝地址：https://shop212360197.taobao.com
	*	QQ交流群：536665479
	*
>>>>> 重要说明：
	*
	*	1. FK743M2-IIT6 核心板 使用的是外部SDRAM作为显存，起始地址0xC0000000，大小为32MB
	*	2. 在刚下载完程序时，屏幕有轻微抖动和闪烁属于正常现象，等待片刻或者重新上电即可恢复正常
	* 	3. LTDC时钟在 main.c 文件里的 SystemClock_Config()函数设置，配置为10MHz，即刷新率在60帧左右，过高或者过低都会造成闪烁
	* 
>>>>> 其他说明：
	*
...
/*----------------------------- 24位色 RGB888 模式 -------------------------*/	
	...
	{		
		printf("lcd_d:%hhu,dh:%hhu\n",LCD.Direction,Direction_H);
		if (LCD.Direction == Direction_H) //水平方向
		{
